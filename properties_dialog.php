...
$var=1;
if(!is_set($arCurrentValues)){
	$arCurrentValues=[];
}
if(empty($arCurrentValues["write2file"])){
	$arCurrentValues["write2file"]='/bitrix/test.log';
}
?>
<script>
    var b_log_counter = -1;

        console.log('runnner 11');
    var f_table = document.getElementById('write2filetable');
    console.log('f_table:',f_table);
    ...
    function Write2File() {
        ...
        if (f_table.rows.length <= 0) {
            ...
            newCell.innerHTML = '<input size="40" type="text" id="path2file" name="path2file" value="<? echo ($arCurrentValues["path2file"]) ? $arCurrentValues["path2file"] : "/bitrix/test.log"?>">';
        } else
            f_table.deleteRow("-1");
    }
</script>

<tr id="write_file_form" style="display:line" idn="runnner">
    <td colspan="2">
        <input type="checkbox" id="write2file" name="write2file"
               onchange="Write2File();"><?= GetMessage("WRITE_2_LOG"); ?>
        <table width="100%" border="0" cellpadding="2" cellspacing="2" id="write2filetable">
        </table>
    </td>
</tr>

<tr id="pd_list" style="display:line" idns="runnner">
    <td colspan="2">
